...
const SpeciesConstructor = require("especially/abstract-operations").SpeciesConstructor;
...
const atAtSpecies = require("especially/well-known-symbols")["@@species"];
...
function PerformPromiseThen(promise, onFulfilled, onRejected, onCanceled, resultCapability) {
  ...
  if (IsCallable(onCanceled) === false) {
    onCanceled = "Identity"; // TODO this is no good; needs to "throw cancel"
  }
  ...
  const cancelReaction = {
    Handler: onCanceled,
    DefaultReaction: resultCapability.Cancel,
    ThrowReaction: resultCapability.Reject
  };
