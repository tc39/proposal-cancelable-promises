...
function FulfillPromise(promise, value) {
  ...
  const finallyReactions = get_slot(promise, "[[PromiseFinallyReactions]]");
  ...
  set_slot(promise, "[[PromiseFinallyReactions]]", undefined);
  set_slot(promise, "[[PromiseState]]", "fulfilled");

  return TriggerPromiseReactions(promise, reactions, finallyReactions, value);
}

function CancelPromise(promise) {
  assert(get_slot(promise, "[[PromiseState]]") === "pending");
  
  const reactions = get_slot(promise, "[[PromiseFinallyReactions]]");
...
function RejectPromise(promise, reason) {
  ...
  return TriggerPromiseReactions(promise, reactions, finallyReactions, reason);
}

export function TriggerPromiseReactions(promise, reactions, finallyReactions, argument) {
  for (const reaction of reactions) {
    EnqueueJob("PromiseJobs", PromiseReactionJob, [reaction, argument]);
  }

  const propogateCancelation = get_slot(promise, "[[PromiseState]]") === 'canceled';

  for (const finallyReaction of finallyReactions) {
    EnqueueJob("PromiseJobs", PromiseFinallyJob, [finallyReaction, propogateCancelation, argument]);
  }
...
function PromiseFinallyJob(reaction, propogateCancelation, finalValue) {
  const promiseCapability = reaction.Capabilities;
  const handler = reaction.Handler;

  if (handler === "Identity") {
    Call(promiseCapability.Resolve, undefined, [finalValue]);
  }
  else {
    let threw = false;
    let handlerResult = undefined;
    ...
    try {
      handlerResult = Call(handler, undefined);
    }
    catch (callE) {
      threw = true;
      Call(promiseCapability.Reject, undefined, [callE]);
    ...
    function handleResolve() {
      if (propogateCancelation) {
        Call(promiseCapability.Cancel, undefined, []);
      }
      else {
        Call(promiseCapability.Resolve, undefined, [finalValue]);
      }
    ...
    if (!threw) {
      // TODO: make this the same check as promiseCapability.Resolve
      if (handlerResult && handlerResult.then && IsCallable(handlerResult.then)) {
        // TODO: make this spec-like
        handlerResult = handlerResult.then(handleResolve, promiseCapability.Reject);
        Call(promiseCapability.Resolve, undefined, [handlerResult]);
      }
      else {
        handleResolve();
...
export default class Promise {
  constructor(executor) {
    ...
    make_slots(promise, ["[[PromiseState]]", "[[PromiseConstructor]]", "[[PromiseResult]]",
      "[[PromiseFulfillReactions]]", "[[PromiseRejectReactions]]", "[[PromiseFinallyReactions]]"]);
    ...
    set_slot(promise, "[[PromiseFinallyReactions]]", []);
  ...
  then(onFulfilled, onRejected) {
    ...
    return PerformPromiseThen(promise, onFulfilled, onRejected, false, resultCapability);
  }
    ...
    const C = SpeciesConstructor(promise, Promise);
    const resultCapability = NewPromiseCapability(C);
    return PerformPromiseThen(promise, false, false, onFinally, resultCapability);
  }
}

export function PerformPromiseThen(promise, onFulfilled, onRejected, onFinally, resultCapability) {
  ...
  const finallyReaction = { Capabilities: resultCapability, Handler: onFinally };
  ...
  if (get_slot(promise, "[[PromiseState]]") === "pending") {
    ...
    get_slot(promise, "[[PromiseFinallyReactions]]").push(finallyReaction);
  } else if (get_slot(promise, "[[PromiseState]]") === "fulfilled") {
    const value = get_slot(promise, "[[PromiseResult]]");
    EnqueueJob("PromiseJobs", PromiseReactionJob, [fulfillReaction, value]);
    EnqueueJob("PromiseJobs", PromiseFinallyJob, [finallyReaction, false, reason]);
    ...
    EnqueueJob("PromiseJobs", PromiseFinallyJob, [finallyReaction, true, reason]);
  }
