...
export function TriggerPromiseReactions(promise, reactions, argument) {
  ...
  for (const onFinally of get_slot(promise, "[[PromiseFinallyCallbacks]]")) {
    EnqueueJob("PromiseJobs", PromiseFinallyJob, [onFinally, argument]);
  }
...
function PromiseFinallyJob(reaction, finalValue) {
  const promiseCapability = reaction.Capabilities;
  const handler = reaction.Handler;
  ...
  if (handler === "Identity") {
    ...
    if (!threw) {
      ...
      if (handlerResult && handlerResult.then && IsCallable(handlerResult.then)) {
        handlerResult = handlerResult.then(() => finalValue);
        Call(promiseCapability.Resolve, undefined, [handlerResult]);
      }
      else {
        Call(promiseCapability.Resolve, undefined, [finalValue]);
...
export default class Promise {
  constructor(executor) {
    ...
    make_slots(promise, ["[[PromiseState]]", "[[PromiseConstructor]]", "[[PromiseResult]]",
      "[[PromiseFulfillReactions]]", "[[PromiseRejectReactions]]", "[[PromiseFinallyCallbacks]]"]);
    ...
    set_slot(promise, "[[PromiseFinallyCallbacks]]", []);
...
// TODO: should this be part of "PerformPromiseThen" even though we don't allow
// finallies to be attached via .then()?
export function PerformPromiseFinally(promise, onFinally, resultCapability) {
  assert(IsPromise(promise) === true);
  ...
  if (get_slot(promise, "[[PromiseState]]") === "pending") {
    get_slot(promise, "[[PromiseFinallyCallbacks]]").push(finallyReaction);
  }
  else {
    const reason = get_slot(promise, "[[PromiseResult]]");
    EnqueueJob("PromiseJobs", PromiseFinallyJob, [finallyReaction, reason]);
  }
