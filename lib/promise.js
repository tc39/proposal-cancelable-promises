  ...
  return {
    Resolve: resolution => {
      ...
      if (IsPromise(resolution) === true) {
        resolution = FollowPromiseResolutionChain(resolution);
      }
      ...
      set_slot(promise, "[[PromiseFollowing]]", resolution);
...
function FollowPromiseResolutionChain(resolution) {
  while (has_slot(resolution, "[[PromiseFollowing]]")) {
    const result = get_slot(resolution, "[[PromiseFollowing]]");
    if (result !== undefined) {
      resolution = result;
    } else {
      return resolution;
...
export default class Promise {
  constructor(executor) {
    ...
    make_slots(promise, ["[[PromiseState]]", "[[PromiseConstructor]]", "[[PromiseResult]]",
      "[[PromiseFulfillReactions]]", "[[PromiseRejectReactions]]", "[[PromiseFollowing]]"]);
